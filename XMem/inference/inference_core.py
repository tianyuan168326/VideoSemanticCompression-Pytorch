...
class InferenceCore:
    ...
    def step(self, image, mask=None, valid_labels=None, end=False):
        ...
        key, shrinkage, selection, f16, f8, f4,bpp_k_frame = self.network.encode_key(image, 
                                                                         self.curr_ti,
        ...
        # print(self.curr_ti,is_mem_frame)
        # save as memory if needed
        bpp_v = 0
        if is_mem_frame:
            value, hidden,bpp_v = self.network.encode_value(image,self.curr_v_ti, f16, self.memory.get_hidden(), 
        ...
        print(bpp_k_frame,bpp_v)
        return unpad(pred_prob_with_bg, self.pad),bpp_k_frame+bpp_v
