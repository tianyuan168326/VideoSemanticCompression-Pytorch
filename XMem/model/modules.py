...
class ValueEncoder(nn.Module):
    def __init__(self, value_dim, hidden_dim, quantization_m, single_object=False):
        super().__init__()
    ...
    def forward(self, image, frame_id,image_feat_f16, h, masks, others, is_deep_update=True):
        # image_feat_f16 is the feature from the key encoder
        if not self.single_object:
            g = torch.stack([masks, others], 2)
        else:
            g = masks.unsqueeze(2)
        # print(image.size(), g.size())
        # exit()
        ...
        bpp_v_frame =torch.zeros([1])
        # if feat_comp:
        #     _,_,fh,fw = g.size()
        #     if frame_id %10== 0:
        #         self.feature_buffer = []
        #     self.feature_buffer+=[g]
        #     faeture_list = self.feature_buffer
        #     faeture_list_vid = torch.stack(faeture_list,dim=2)
        #     faeture_list_vid_comp,bpp = self.quantization_m(faeture_list_vid)
        #     if frame_id>0 and frame_id %9 == 0:
        #         bpp_v_frame = bpp/(fh*fw*16*16)
        #     else:
        #         bpp_v_frame = bpp*0/(fh*fw*16*16)
        #     g = faeture_list_vid_comp[:,:,-1]
        #     bpp_v_frame = torch.mean(bpp_v_frame,0,True)
        ...
        return g, h,bpp_v_frame
 

class KeyEncoder(nn.Module):
    def __init__(self,quantization_m):
        super().__init__()
    ...
    def forward(self, f,frame_id):
        x = self.conv1(f) 
        x = self.bn1(x)
        x = self.relu(x)   # 1/2, 64
        x = self.maxpool(x)  # 1/4, 64
        f4 = self.res2(x)   # 1/4, 256
        bpp_k_frame = torch.zeros([1])
        # if feature_comp:
        #     _,_,fh,fw = f4.size()
        #     if frame_id %10== 0:
        #         self.feature_buffer = []
        #     self.feature_buffer+=[f4]
        #     faeture_list = self.feature_buffer
        #     faeture_list_vid = torch.stack(faeture_list,dim=2)
        #     faeture_list_vid_comp,bpp = self.quantization_m(faeture_list_vid)
        #     if frame_id>0 and frame_id %9 == 0:
        #         bpp_k_frame = bpp/(fh*fw*4*4)
        #         print(bpp,frame_id)
        #     else:
        #         bpp_k_frame = bpp*0/(fh*fw*4*4)
        #     f4 = faeture_list_vid_comp[:,:,-1]
        
        f8 = self.layer2(f4) # 1/8, 512
        f16 = self.layer3(f8) # 1/16, 1024

        return f16, f8, f4,bpp_k_frame
