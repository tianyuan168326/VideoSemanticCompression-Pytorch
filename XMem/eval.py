...
parser.add_argument('--test_compress_q', help='test_compress_q', default='31')
...
for vid_reader in progressbar(meta_loader, max_value=len(meta_dataset), redirect_stdout=True):
    ...
    for ti, data in enumerate(loader):
        with torch.cuda.amp.autocast(enabled=not args.benchmark):
            ...
            prob,bpp = processor.step(rgb, msk, labels, end=(ti==vid_length-1))
            total_bpps += [bpp]
...
print( "total average bpp",sum(total_bpps)/len(total_bpps)  )
