...
class GroupRandomHorizontalFlip(object):
    ...
    def __call__(self, img):
        ret,label = img
        v = random.random()
        v_topdown = random.random()
        v_rot = random.random()
        if v < 0.5:
            ret = [img.transpose(Image.FLIP_LEFT_RIGHT) for img in ret]
        if v_topdown<0.5:
            ret = [img.transpose(Image.FLIP_TOP_BOTTOM) for img in ret]
        # if v_rot>0 and v_rot<0.33:
        # 	ret = [img.transpose(Image.ROTATE_90) for img in ret]
        # elif v_rot>0.33 and v_rot<0.66:
        # 	ret = [img.transpose(Image.ROTATE_180) for img in ret]
        # else:
        # 	ret = [img.transpose(Image.ROTATE_270) for img in ret]	
...
class FullSample(object):
    ...
    def __call__(self, img_group):
        ...
        image_w, image_h = img_group[0].size
        crop_w, crop_h = self.crop_size
        # print(image_w, image_h, crop_w, crop_h)
        ...
        # offsets.append((4 * w_step, 2 * h_step))  # right
        # offsets.append((2 * w_step, 2 * h_step))  # center
        # ofs = random.sample(offsets, 1)[0]

        if  image_h - crop_h <0:
            o_h = 0
        else:
            o_h = random.randint(0, image_h - crop_h)
        o_w = random.randint(0, image_w - crop_w)
        ...
        for i, img in enumerate(img_group):
            if o_h == 0:
                ...
                padding_height = max(desired_height - current_height, 0)  # Only add padding if needed
                padding_tuple = (0, 0, 0, padding_height)  # left, top, right, bottom
                img = ImageOps.expand(img, padding_tuple, fill="white")
            crop = img.crop((o_w, o_h, o_w + crop_w, o_h + crop_h))
            normal_group.append(crop)
            # if self.flip:
            # 	flip_crop = crop.copy().transpose(Image.FLIP_LEFT_RIGHT)
