    ...
    def multi_gpu_test(  # noqa: F811
        ...
        for data in data_loader:
            with torch.no_grad():
                result = model(return_loss=False, **data)
            # print(result)
            # print(result["cls_score"].shape,result["h265_bitcosts"].shape,result["code_bpp"].shape)
            # print(result["h265_bitcosts"].shape)
            # exit()
            results_cls_score.extend(result["cls_score"])
            results_h265_bitcosts.extend(result["h265_bitcosts"])
            results_code_bpp.extend(result["code_bpp"])
            # results.extend(result)
        ...
        if gpu_collect:
            results_cls_score = collect_results_gpu(results_cls_score, len(dataset))
            results_h265_bitcosts = collect_results_gpu(results_h265_bitcosts, len(dataset))
            results_code_bpp = collect_results_gpu(results_code_bpp, len(dataset))
        else:
            results_cls_score = collect_results_cpu(results_cls_score, len(dataset), tmpdir)
            results_h265_bitcosts = collect_results_cpu(results_h265_bitcosts, len(dataset), tmpdir)
            results_code_bpp = collect_results_cpu(results_code_bpp, len(dataset), tmpdir)
        
        return {
            "cls_score": results_cls_score,
            "h265_bitcosts":results_h265_bitcosts,
            "code_bpp":results_code_bpp
        }
