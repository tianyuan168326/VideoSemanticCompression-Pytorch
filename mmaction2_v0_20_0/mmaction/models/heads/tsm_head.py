    ...
    def forward(self, x, num_segs):
        ...
        if self.is_shift and self.temporal_pool:
            ...
            cls_score = cls_score.view((-1, self.num_segments // 2) +
                                       cls_score.size()[1:])
            # h265_bitcosts = h265_bitcosts.view((-1, self.num_segments // 2) +
            #                            h265_bitcosts.size()[1:])
            # code_bpp = code_bpp.view((-1, self.num_segments // 2) +
            #                            code_bpp.size()[1:])
        else:
            # [N, num_segs, num_classes]
            cls_score = cls_score.view((-1, self.num_segments) +
                                       cls_score.size()[1:])
            # h265_bitcosts = h265_bitcosts.view((-1, self.num_segments) +
            #                            h265_bitcosts.size()[1:])
            # code_bpp = code_bpp.view((-1, self.num_segments) +
            #                            code_bpp.size()[1:])
        # [N, 1, num_classes]
        cls_score = self.consensus(cls_score)
        # [N, num_classes]
        # print(cls_score.size(), h265_bitcosts.size(), code_bpp.size())
        # exit(0)
        # h265_bitcosts = h265_bitcosts.mean(1)
        # code_bpp = code_bpp.mean(1)
        # return cls_score.squeeze(1),var1,var2
        return {
            "cls_score":cls_score.squeeze(1),
            "h265_bitcosts":h265_bitcosts,### b 1
            "code_bpp":code_bpp,## b 1
        }
