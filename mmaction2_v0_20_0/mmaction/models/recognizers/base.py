...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    @auto_fp16()
    def extract_feat(self, imgs,imgs_compress,img_norm_cfg):
        ...
        if (hasattr(self.backbone, 'features')
            ...
            x = self.backbone(imgs,imgs_compress,img_norm_cfg)
    ...
    def average_clip(self, cls_score, num_segs=1):
        ...
        return cls_score
    def average_bpp(self, bpp_scores, num_segs=1):
        
        

        batch_size = bpp_scores.shape[0]
        bpp_scores = bpp_scores.view(batch_size // num_segs, num_segs)

        
        bpp_scores = bpp_scores.mean(dim=1)
    ...
    def forward(self, imgs,imgs_compress,img_norm_cfg, label=None, return_loss=True, **kwargs):
        ...
        if return_loss:
            ...
            return self.forward_train(imgs,imgs_compress,img_norm_cfg, label, **kwargs)
        
        return self.forward_test(imgs,imgs_compress, img_norm_cfg,**kwargs)
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        imgs_compress = data_batch['imgs_compress']
        img_norm_cfg = data_batch['img_norm_cfg']
        ...
        losses = self(imgs,imgs_compress,img_norm_cfg, label, return_loss=True, **aux_info)
