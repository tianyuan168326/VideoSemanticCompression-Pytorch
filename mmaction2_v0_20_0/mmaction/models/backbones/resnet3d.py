...
class ResNet3d(nn.Module):
    ...
    def __init__(self,
        ...
        self.depth = depth
        self.test_mode = test_mode ## hd raw lbvu
        ...
        ### LBVU
        if self.test_mode == 'lbvu':
            from generators import DiffCompensate
            ...
            if model_version == 'v1':
                opt = {
                    ...
                    "restortion_net":"r_net",
                    "lambda_bpp":1,
                    "last_Tlayer_causual":False,
                    "sr_scale" : 1,
                    "K_enc":32
                }
            opt = DefaultMunch.fromDict(opt)
            self.pre_code_net = DiffCompensate(opt)
            if pretrained_lbvu_path == None:
                pretrained_lbvu_path = "/data_video/code/lbvu/checkpoints/TSM_50/train_vqgan_l4_seg_2dgan_wotrans/checkpoint/action_backbone.TSM_k60k_brrand_resnet50_segment8__best_network.pth.tar"
            if ".pth" in pretrained_lbvu_path: ## if is a model file
                ...
                self.pre_code_net.load_state_dict(ckeck_copy,strict=False)
    ...
    def _init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str):
            ...
            for n,m in self.named_modules():
                if "pre_code_net" in n or "saliency_net" in n or 'fvc_net' in n:
                    print("passing",n)
                    continue
    ...
    def forward(self, x,x_compress,img_norm_cfg):
        ...
        """
        if self.test_mode == 'hd':
            
            code_bpp = torch.zeros((x.size(0)//1,1))
            h265_bitcosts = torch.zeros((x.size(0)//1,1))
        elif self.test_mode == 'raw':
            mean = img_norm_cfg['mean']/255.0
            std = img_norm_cfg['std']/255.0
            std = std[0:1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            mean = mean[0:1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            # print(x.size(),std.size())
            x_pre_norm = x *std + mean
            # imgs = torchvision.utils.make_grid(x_pre_norm[0].transpose(0,1),nrow=8,normalize =False)
            # torchvision.utils.save_image(imgs,"1.jpg")
            # print(x_pre_norm.max(),x_pre_norm.min())
            # exit()
            input_clean = x_pre_norm
            input_com,h265_bitcosts = self.quantization_m(input_clean)
            input_com  =input_com.cuda(input_clean.device)
            # code,code_bpp  = self.pre_code_net([input_clean,input_com],'enc')
            h265_bitcosts = h265_bitcosts.unsqueeze(-1)
            code_bpp = torch.zeros((h265_bitcosts.size(0),1))
            # return code
            # recons_clean ,aux_vid,total_nce_loss,l1_loss = self.pre_code_net([code,input_com,input_clean],'dec')
            # imgs = torchvision.utils.make_grid(recons_clean[0].transpose(0,1),nrow=8,normalize =False)
            # torchvision.utils.save_image(imgs,"1.jpg")
            x = (input_com-mean)/std
        elif self.test_mode == 'lbvu':
            mean = img_norm_cfg['mean']/255.0
            std = img_norm_cfg['std']/255.0
            std = std[0:1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            mean = mean[0:1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        ...
        elif self.test_mode == 'lbvu_train':
            mean = img_norm_cfg['mean']/255.0
            std = img_norm_cfg['std']/255.0
            std = std[0:1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            mean = mean[0:1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if i in self.out_indices:
                outs.append(x)
        if len(outs) == 1:
            # return outs[0],h265_bitcosts,code_bpp
            return {
                "cls_score": outs[0],
