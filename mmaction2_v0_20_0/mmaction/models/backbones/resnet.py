...
class ResNet(nn.Module):
    ...
    def forward(self, x,x_compress,img_norm_cfg):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            x = res_layer(x)
            if self.test_mode == 'feature_comp' and i == 2:
                _x  = x
                bt,c,h,w= x.size()
                x_3d = x.reshape(b,clip_length,c,h,w)
                x_3d,h265_bitcosts = self.quantization_m(x_3d.transpose(1,2))
                x = x_3d.transpose(1,2).reshape(bt,c,h,w)
            if i in self.out_indices:
                outs.append(x)
        if len(outs) == 1:
            # outs[0],h265_bitcosts,code_bpp
            return {
                "cls_score": outs[0],
    ...
    def train(self, mode=True):
        ...
        super().train(mode)
        self._freeze_stages()
        # if "lbvu_train" in self.test_mode:
        # 	self.
