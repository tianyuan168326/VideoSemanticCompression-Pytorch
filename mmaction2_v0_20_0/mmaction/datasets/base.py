...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.data_prefix_compress = osp.realpath(
            data_prefix_compress) if data_prefix_compress is not None and osp.isdir(
                data_prefix_compress) else data_prefix_compress
    ...
    def evaluate(self,
        ...
        results = results_tuple["cls_score"]
        assert len(results) == len(self), (
            f'The length of results is not equal to the dataset len: '
            f'{len(results)} != {len(self)}')
        codec_results = results_tuple["h265_bitcosts"]
        meta_results = results_tuple["code_bpp"]
        # print(codec_results)
        # results = np.concatenate(results)
        # codec_results = np.concatenate(codec_results)
        # meta_results = np.concatenate(meta_results)
        # print(results.shape, codec_results.shape,meta_results.shape)        
        ...
        for metric in metrics:
            ...
            if metric == 'top_k_accuracy':
                ...
                for k, acc in zip(topk, top_k_acc):
                    ...
                    log_msg.append(f'\ntop{k}_acc\t{acc:.4f}')
                # for codec_b, meta_b in zip(codec_results, meta_results):
                # print(codec_results.shape)
                codec_b = sum(codec_results)/len(codec_results)
                codec_b = codec_b.item()
                meta_b = sum(meta_results)/len(meta_results)
                meta_b = meta_b.item()

                eval_results[f'codec_b'] = codec_b
                eval_results[f'meta_b'] = meta_b
                log_msg.append(f'\ncodec_b\t{codec_b:.4f}')
                log_msg.append(f'\nmeta_b\t{meta_b:.4f}')
