...
class RandomCrop:
    ...
    def __call__(self, results):
        ...
        if not self.lazy:
            ...
            if 'imgs_compress' in results:
                results['imgs_compress'] = self._crop_imgs(results['imgs_compress'], crop_bbox)
        else:
            lazyop = results['lazy']
            if lazyop['flip']:
...
class Resize:
    ...
    def __call__(self, results):
        ...
        ## convert to the interger times of 32
        interval = 32
        new_w = interval* (new_w//interval)
        new_h = interval* (new_h//interval)
        ...
        if not self.lazy:
            if 'imgs' in results:
                results['imgs'] = self._resize_imgs(results['imgs'], new_w,
                                                    new_h)
            if 'imgs_compress' in results:
                results['imgs_compress'] = self._resize_imgs(results['imgs_compress'], new_w,
                                                    new_h)
            if 'keypoint' in results:
            ...
            if flip:
                if 'imgs' in results:
                    results['imgs'] = self._flip_imgs(results['imgs'],
                                                      modality)
                if 'imgs_compress' in results:
                    results['imgs_compress'] = self._flip_imgs(results['imgs_compress'],
                                                      modality)
                if 'keypoint' in results:
                    kp = results['keypoint']
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        if modality == 'RGB':
            ...
            ##compress
            n = len(results['imgs_compress'])
            h, w, c = results['imgs_compress'][0].shape
            imgs_compress = np.empty((n, h, w, c), dtype=np.float32)
            for i, img in enumerate(results['imgs_compress']):
                imgs_compress[i] = img

            for img in imgs_compress:
                mmcv.imnormalize_(img, self.mean, self.std, self.to_bgr)
            results['imgs_compress'] = imgs_compress
            results['img_norm_cfg'] = dict(
                mean=self.mean, std=self.std, to_bgr=self.to_bgr)
            return results
        if modality == 'Flow':
...
class ThreeCrop:
    ...
    def __call__(self, results):
        ...
        imgs = results['imgs']
        imgs_compress = results['imgs_compress']
        ...
        for x_offset, y_offset in offsets:
            ...
            cropped.extend(crop)
            cropped_compress.extend(crop_compress)
            crop_bboxes.extend([bbox for _ in range(len(imgs))])

        crop_bboxes = np.array(crop_bboxes)
        results['imgs'] = cropped
        results['imgs_compress'] = cropped_compress
