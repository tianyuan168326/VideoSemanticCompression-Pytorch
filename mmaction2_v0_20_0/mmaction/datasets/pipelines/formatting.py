...
class FormatShape:
    ...
    def __call__(self, results):
        ...
        if not isinstance(results['imgs_compress'], np.ndarray):
            results['imgs_compress'] = np.array(results['imgs_compress'])
        imgs = results['imgs_compress']
        # [M x H x W x C]
        # M = 1 * N_crops * N_clips * L
        if self.collapse:
            assert results['num_clips'] == 1
