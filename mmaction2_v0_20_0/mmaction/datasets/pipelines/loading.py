...
class DecordInit:
    ...
    def __call__(self, results):
        ...
        results['video_reader'] = container
        file_obj = io.BytesIO(self.file_client.get(results['filename_compress']))
        container = decord.VideoReader(file_obj, num_threads=self.num_threads)
        results['video_reader_compress'] = container
        results['total_frames'] = len(container)
        return results
...
class DecordDecode:
    ...
    def __call__(self, results):
        ...
        """
        container = results['video_reader']
        container_compress = results['video_reader_compress']
        ...
        if self.mode == 'accurate':
            ...
            imgs_compress = container_compress.get_batch(frame_inds).asnumpy()
            imgs_compress = list(imgs_compress)
        elif self.mode == 'efficient':
...
class RawFrameDecode:
    ...
    def __call__(self, results):
        ...
        directory_compress = results['frame_dir_compress']
        ...
        for i, frame_idx in enumerate(results['frame_inds']):
            ...
            if frame_idx in cache:
                if modality == 'RGB':
                    ...
                    imgs_compress.append(cp.deepcopy(imgs_compress[cache_compress[frame_idx]]))
                else:
            ...
            if modality == 'RGB':
                ...
                filepath = osp.join(directory_compress, filename_tmpl.format(frame_idx))
                img_bytes = self.file_client.get(filepath)
                # Get frame with channel order RGB directly.
                cur_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')
                imgs_compress.append(cur_frame)
            # elif modality == 'Flow':
